...
public class ProjectileLauncher : MonoBehaviour
{
    ...
    public void StartTravelPath(Vector3 casterTilePos_, Vector3 targetTilePos_)
    {
        ...
        {
            this.yVelocityInterp = new Interpolator(EaseType.SineOut, this.travelTime/2);
        }

        //Setting our position to the caster tile position
        this.transform.localPosition = new Vector3(this.castersTilePos.x, this.castersTilePos.y, -200);

        //If we need to rotate, then we do
        if(this.rotateToFaceTarget)
        {
            float newAngle = Vector2.Angle(new Vector2(this.castersTilePos.x, this.castersTilePos.y), new Vector2(this.targetTilePos.x, this.targetTilePos.y));
    ...
    private void Update()
    {
        ...
        switch(this.pathType)
        {
            case ProjectilePath.Straight://If we travel in a straight line to the target
                ...
                this.transform.localPosition = new Vector3(xDiff, yDiff, -200);
                break;
            ...
            case ProjectilePath.Arc://If we lob this projectile at the target
                ...
                if(this.rotateToFaceTarget)
                {
                    float newAngle = Vector2.Angle(new Vector2(this.transform.localPosition.x, this.transform.localPosition.y), new Vector2(xDiff, yDiff));
                    this.transform.eulerAngles = new Vector3(0, 0, newAngle);
                }
                //Setting our new position using the x and y differences
                this.transform.localPosition = new Vector3(xDiff, yDiff, -200);
